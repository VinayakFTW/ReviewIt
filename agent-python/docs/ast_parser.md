...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:88*

Extract lines [start, end] (1-indexed) from source.

### `def _unparse_annotation(node) -> str:`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:93*
...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:100*

Reconstruct def line(s) from AST rather than regex.

### `def _collect_calls(func_node: ast.AST) -> List[str]:`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:112*

Walk function body and collect all called names.

### `def _get_decorators(node: ast.FunctionDef) -> List[str]:`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:124*

Extracts decorator names from an AST FunctionDef node.

Parameters:
- node (ast.FunctionDef): The function definition node from which to extract decorators.

Returns:
List[str]: A list of decorator names or attribute expressions as strings.

Raises:
None.

### `def parse_file(filepath: str) -> FileAnalysis:`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:287*
...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:396*
...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:153*

Initializes a new instance of the class with the provided source lines and file path. Sets up internal lists for findings and function stack.

Parameters:
- `source_lines`: A list of strings representing the lines of source code.
- `filepath`: A string representing the path to the source file.
...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:161*

Emits a static code analysis finding.

Parameters:
- line (int): The line number where the issue was found.
- severity (str): The severity level of the finding (e.g., "error", "warning").
- rule (str): The rule that was violated.
- msg (str): A message describing the issue.
...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:173*

Visits an ExceptHandler node in the AST. Checks for a bare except clause and emits a warning if found. Parameters: node (ast.ExceptHandler). Returns: None. Raises: No exceptions raised by this function.

### `def visit_FunctionDef(self, node: ast.FunctionDef):`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:183*

Visits a function definition node in the abstract syntax tree (AST) and checks for several issues such as long functions, mutable default arguments, and missing return annotations. Emits warnings or errors based on these checks.
...
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:218*

Visits an AST Call node to check for security issues such as the use of eval/exec and os.system. Emits warnings if these functions are detected. Parameters: node (ast.Call) - The AST Call node to visit. Returns: None Raises: None

### `def visit_Assign(self, node: ast.Assign):`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:242*

Visits an assignment node to check for hardcoded secrets. Parameters: node (ast.Assign). Returns: None. Raises: No exceptions are raised by this function.

### `def visit_JoinedStr(self, node: ast.JoinedStr):`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:254*

Visits a JoinedStr node to check for potential SQL injection vulnerabilities. If the unparsed string matches an SQL pattern, emits a high-severity warning. Parameters: node (ast.JoinedStr). Returns: None. Raises: Exception if ast.unparse fails.

### `def visit_Import(self, node: ast.Import):`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:268*

Visits an Import node in the AST. Handles wildcard imports separately. Parameters: node (ast.Import). Returns: None. Raises: No exceptions raised.

### `def visit_ImportFrom(self, node: ast.ImportFrom):`
*D:\CodeSentinel\agent-python\ingest\ast_parser.py:272*
